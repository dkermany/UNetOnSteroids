...
class BaseMaskDatasetIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for batch_index, image_index in enumerate(index_array):
            ...
            if len(good4copy) > 0 and random.random() > 0.92:
...
if __name__ == '__main__':
    ...
    for it in range(4):
        ...
        train_msk = (df['fold'] != it) | (df['img_id'].isin(all_folds_ids)) | (df['source'] == 'wikimedia')
        train_idx0 = df[train_msk].index.values
        train_groups = df[train_msk]['cluster'].values
        train_ids = df[train_msk]['img_id'].values
